...
| `geistr:send-message` | Renderer → Main | `string` | When idle, persists a user message, runs the runtime, persists assistant text when available, and returns updated state. When a foreground run is active, text-only submissions are persisted as steering messages and sent through Pi `steer()` so they enter the ongoing working loop before the next model call. Runtime/provider failures are normalized into structured chat errors, with raw details kept for logs/debug details. |
